...
<div class="w-full overflow-x-auto">
    ...
    @if(!empty($detailProject))
        <form method="POST" action="{{ route('participant.detailProject.update',$detailProject['id']) }}" class="px-4 py-3" >
        @method('PUT')
        @csrf
            <!-- link_project -->
            <div class="my-2">
                ...
                <input
                        value="@if(!empty($detailProject)){{$detailProject['link_project']}}@else{{ old('link_project') }}@endif"
                        type="url"
                        required
                        name="link_project"
    ...
    @else
        <div class="px-4 py-2" >
                <p class="text-red-600 text-center font-bold text-md" >Isi terlebih dahulu data tim</p>
        </div>
    @endif
